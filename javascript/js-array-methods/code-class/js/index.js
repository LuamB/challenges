...
//     // console.log("movie:", movie.title, idx);
...
console.log("upperCaseTitles.length", upperCaseTitles.length);
...
// const doubledMap = numbers.map((number) => {
//     return number * 2;
// });
// console.log("doubledMap: ", doubledMap);

/* -------------- FILTER -------------- */
const bestRankedMovies = movies.filter((movie) => {
    return movie.imdbRating > 8;
});
const bestRankedMoviesLongVersion = movies
    .filter((movie) => {
        if (movie.imdbRating > 8) {
            return true;
        } else {
...
console.log("bestRankedMoviesLongVersion: ", bestRankedMoviesLongVersion);
